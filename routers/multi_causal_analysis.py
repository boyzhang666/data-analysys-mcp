...
def calculate_granger_causality_matrix(data_dict: Dict[str, np.ndarray], max_lag: int, significance_level: float) -> Dict[str, Dict[str, float]]:
    """计算格兰杰因果关系矩阵"""
        ...
        for j, var2 in enumerate(variables):
            if i == j:
                ...
                try:
                    ...
                    # 计算因果强度（1 - p值）
                    causal_strength = max(0, 1 - min_p_value) if min_p_value < significance_level else 0.0
                    causal_matrix[var1][var2] = causal_strength
                    
                except Exception as e:
                    logger.warning(f"计算{var1}->{var2}的格兰杰因果关系时出错: {str(e)}")
                    causal_matrix[var1][var2] = 0.0
    
    return causal_matrix


def calculate_correlation_matrix(data_dict: Dict[str, np.ndarray]) -> Dict[str, Dict[str, float]]:
    """计算相关性矩阵"""
    variables = list(data_dict.keys())
    correlation_matrix = {}
        ...
        for var2 in variables:
            if var1 == var2:
                ...
                try:
                    ...
                    correlation_matrix[var1][var2] = abs(corr) if not np.isnan(corr) else 0.0
                except Exception:
...
def identify_significant_relationships(causal_matrix: Dict[str, Dict[str, float]], threshold: float = 0.3) -> List[Dict[str, Any]]:
    """识别显著的因果关系"""
        ...
        for effect, strength in effects.items():
            if strength > threshold:
                relationship = {
                    ...
                    "effect": effect,
                    "strength": strength,
                    "significance": "强" if strength > 0.7 else "中等" if strength > 0.5 else "弱"
                }
...
async def multi_causal_analysis(request: MultiCausalAnalysisRequest):
    ...
    **使用示例：**
    ...
    try:
        ...
        # 执行因果分析
        if method == "granger":
            causal_matrix = calculate_granger_causality_matrix(np_data, max_lag, significance_level)
        ...
        if significant_relationships:
            strong_relationships = [r for r in significant_relationships if r["strength"] > 0.7]
        ...
        if significant_relationships:
            ...
            if cause_counts:
                ...
                recommendations.append(f"'{top_cause}'是最重要的影响因子，影响{cause_counts[top_cause]}个其他测点")
